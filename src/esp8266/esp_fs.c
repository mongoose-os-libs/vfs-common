...
bool esp_fs_mount2(uint32_t addr, uint32_t size, const char *dev_name,
  ...
  return (mgos_vfs_dev_create_and_register(MGOS_VFS_DEV_TYPE_PART, dev_opts,
                                           dev_name) &&
          mgos_vfs_mount_dev_name(path, dev_name, fs_type, fs_opts));
}

bool esp_fs_mount(uint32_t addr, uint32_t size, const char *dev_name,
                  const char *path) {
  return esp_fs_mount2(addr, size, dev_name, MGOS_VFS_FS_TYPE_SPIFFS, "", path);
}
...
bool mgos_core_fs_init(void) {
  ...
  return (mgos_vfs_dev_create_and_register(MGOS_VFS_DEV_TYPE_SYSFLASH, "",
                                           ESP8266_SYSFLASH_DEV_NAME) &&
          esp_fs_mount2(root_fs_addr, root_fs_size, MGOS_VFS_ROOT_DEV_NAME,
                        mgos_vfs_get_root_fs_type(),
